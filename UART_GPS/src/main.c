...
void extractAndFormatDate(char *data, char *formattedDate) {
    ...
    char date[7];
    // Calculate the starting index for the date by subtracting 15 characters from number of characters in message
    int startIndex = strlen(data) - 15;
...
int main(void) {
    ...
    while (1) {
        ...
        if (new_sensor_data == 1) {
            //send Temp and Hum data to OLED
        ...
        if (received_char == '\n') {
            ...
            if (strncmp(received_data, gprmc_prefix, strlen(gprmc_prefix)) == 0) {
                ...
                // Transmit the formatted data to UART and OLED
